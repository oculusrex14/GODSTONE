...
**What that changed about the round**: no iOS RED could be run and no iOS repair could be measured, so **no iOS production line was touched** — *a RED that cannot be run is not a RED, and a repair that cannot be measured is not a repair*. The GS-STORE-005 arms written before the block were **parked as a re-appliable patch** rather than left in an unbuildable tree.

**So the round's work went to ANDROID-01, whose lane still runs — and it produced a filed RED and a measured refusal to pretend.** The gap is exactly the audit's charge: `beginTrustedHandshake` **is defined and no production line ever calls it**; production reached a physical duplex and stopped. The arm was run **against the untouched tree** and failed with the law's own words (*"the application must begin D2 itself upon the physically-ready relation; ring: <empty>"*) while its negative control passed — 16 tests / 1 failed, kept in `round260-RED.log`.

**And then the repair was written, it turned the RED green, and it BROKE FIVE PRE-EXISTING ARMS** — because those arms drive the four-way exchange **step by step** and call `beginTrustedHandshake` themselves, so the application's own begin takes the relation into the handshake first. **That is the same migration the iOS twin demanded at rounds 190–208**, and on this isle **six courts** drive it by hand. A half-migrated suite is what this programme refuses to leave behind, so **the repair and its courts were parked together**, with the reasoning written down — and every runnable lane is green: android `:mesh` **1198/0/0**, parity **7/7**, symbols **0 unresolved**, courts **635**, digests PASSED.

**Ledger unchanged in substance: OPEN 11 · FIX_SUBMITTED 38 · PARTIAL 5.** ANDROID-01 stays OPEN with a failing witness and a named next step; GS-STORE-005 stays OPEN with its arms parked behind the licence.
...
- **Production already has** the opened SQLite store, the identity repository, the key resolver, the ACK authenticator, the delivery tracker and its SQLite repo — so the store/pump/dispatcher/inbox are constructible, and `AckObligationStore(engine:)` exists.
- **The blocker, measured**: the only concrete `AckSignerSeam` implementations are `ComposedRuntime.TestAckSigner` (its own comment: *"HARNESS SUPPORT AND NOT A DEVICE RESULT"*) and a test-local `TestSigner`; `RecipientInbox(` has no construction outside the harness. **There is no production signer over the pinned local identity** — and building the driver on a harness signer would be the substitution this programme forbids.
- **Next instrument**: check whether a production signer already exists under another name (the search looked for `AckSignerSeam` conformances only — plausibly GS-ACK-001/002's iOS half); if it exists, construct the four owners and subscribe; if not, the signer is a **prerequisite of this finding** and must be named, not stubbed.

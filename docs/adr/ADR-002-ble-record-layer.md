...
**STATUS: ACCEPTED / PHASE C8.4C RECORD LAYER FROZEN; PHASE C8.4D1-A1 LINKINFO AMENDMENT ACCEPTED / FROZEN; PHASE C8.4D1-R2 OPEN** (29 Aug 2026)
Implementation:
...
- Phase C8.4D1-R2 (Production-integrated BLE substrate: orchestration drivers as production state owners, delivery retirement held-set notification, server callback epoch poisoning, pure ATT cache reads, and semantic mutation controls) is OPEN.
...
- Apple CoreBluetooth background-discovery limitation: iOS background mode cannot discover peripherals by service UUID when the app is suspended; discovery requires foreground or specific `CBCentralManager` state restoration.

...
- **Phase C8.4D1 Persistent Duplex BLE Link Substrate:** Partially Implemented / OPEN (pure unsigned-byte lexicographic role election, 13-byte scan response discovery codec, persistent bidirectional GATT client/server abstractions, connection-local record seams, and structural control checker `ci/check_ble_link_substrate_controls.py` enforcing BL01–BL24; OPEN due to stock iOS CoreBluetooth scan-response discovery metadata SPEC BLOCKER).

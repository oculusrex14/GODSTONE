...
- **Phase C8.4D1 Persistent Duplex BLE Link Substrate:** Implemented (pure unsigned-byte lexicographic role election, 13-byte scan response discovery codec, persistent bidirectional GATT client/server abstractions, connection-local record seams, and structural control checker `ci/check_ble_link_substrate_controls.py` enforcing BL01–BL24).
- **Phase C8.4D2 Trusted Handshake Driver:** OPEN (trusted handshake state machine driving C8.4C records over C8.4D1 substrate).

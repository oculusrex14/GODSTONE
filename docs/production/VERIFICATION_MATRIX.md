...
| Android Archive-only LIGHT release (APK/AAB) | `:app:lintLightRelease :app:assembleLightRelease :app:bundleLightRelease` (scoped to :app) + `lightRelease{Runtime,Compile}Classpath` forbidden-edge check + `inspect_android_artifacts.py` + merged-manifest permission check | release-gates.yml `android-archive-only-release` (repo-owned) | PASS (Archive-only binary does not need llama.cpp/model/Oracle; :app ships `implementation(:core)` only, :llm is testImplementation) | release-gates run; `RELEASE_GATES_STATUS.json` `android-archive-only-release`; APK/AAB SHA-256 + namelist + merged manifest uploaded | Unsigned; no on-device/signing evidence (separate external gates) |

...
/**
 * The ONE composition root for the mesh subsystem (Stage 4B / C8.4B).
 *
 * Provides the unified runtime authority graph:
 * - One [Identity] authority for the process (loaded after [PanicWipe.resumeIfPending]);
...
internal object MeshModule {
    ...
    @Provides @Singleton
    fun provideIdentity(@ApplicationContext ctx: Context): Identity {
    ...
    @Provides @Singleton
    fun providePeerIdentityStore(@ApplicationContext ctx: Context): SqlcipherPeerIdentityStore {
        PanicWipe.resumeIfPending(ctx)

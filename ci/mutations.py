...
SEMANTIC = [
    ...
    {'id': 'T37-RC2-ios-routing-tag-gate-is-identity', 'platform': 'swift', 'file': 'ios/Godstone/Sources/GodstoneMesh/RecipientInboxRepository.swift', 'find': '        bump(hintHit ? .hintHit : .hintMiss)', 'replace': '        bump(hintHit ? .hintHit : .hintMiss)\n        guard hintHit else { return .rejected(reason: .notForUs, detail: "tag mismatch") }', 'why': 'the 4-byte rotating tag is promoted from a charged hint to an admission gate: a clock-skewed node whose tag left the +-window can no longer deliver at all', 'witness': 'testStaleTagStillDeliversAndMatchedTagAdmitsNothingAlone', 'swift_filter': 'ReadinessT37Tests'},

...
SEMANTIC = [
    ...
    {'id': "T40-RC1-android-digest-from-seen-union", 'platform': "jvm", 'file': "android/mesh/src/main/java/io/godstone/mesh/router/SyncControlOwner.kt", 'find': "for (id in snap.ids) bloom.add(id)", 'replace': "for (id in (snap.ids + relations.values.flatMap { it.wantQueue })) bloom.add(id)", 'why': "the card's first named negative restored-as-mutation: the advertised filter is built over the seen-but-unheld union -- every received id still waiting in any relation's want queue gains entry in the bloom, so the peer is told the store holds what the durable store has never carried; the advertisement witness convicts by demanding the digest be the captured vector's own bloom, octet for octet", 'witness': "testForcedBloomCollisionConvergesByExactReconciliation", 'gradle_filter': "ReadinessT40Test"},
    {'id': "T40-RC1-ios-digest-from-seen-union", 'platform': "swift", 'file': "ios/Godstone/Sources/GodstoneMesh/SyncControlOwner.swift", 'find': "for id in snap.ids { bloom.add(id) }", 'replace': "for id in snap.ids + relations.values.flatMap({ $0.wantQueue }) { bloom.add(id) }", 'why': "on the second isle the same forgetting: the digest speaks of the queues of every relation as well as the captured vector; the advertisement assertion, that the bloom is the vector's own, is falsified the moment a wanted-but-unheld id lends its bits", 'witness': "testForcedBloomCollisionConvergesByExactReconciliation", 'swift_filter': "ReadinessT40Tests"},

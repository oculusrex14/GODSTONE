...
SEMANTIC = [
    ...
    {"id": "T50-SM3-before-gate-sleepeth", "platform": "swift", "file": "ios/Godstone/Sources/GodstoneCore/ArchiveSceneModel.swift", "court": "ios/Godstone/Tests/GodstoneCoreTests/ReadinessT50Tests.swift", "swift_filter": "ReadinessT50Tests", "find": "        lastRequest = { [weak self] in await self?.search() }\n        if epoch != mine { return }   // stale before the road -- and before any publication", "replace": "        lastRequest = { [weak self] in await self?.search() }\n        if epoch != mine { }   // (mutant) the gate sleepeth", "why": "the token is not consulted before delivery: a superseded petition walketh the road and toucheth the engine at all. The before-gate witness condemneth -- the fake history must stay empty and it is not", "witness": "testTheGateBeforeTheRoadStoppetheTheSupersededPetition"},
    {"id": "T50-SM4-after-gate-sleepeth", "platform": "swift", "file": "ios/Godstone/Sources/GodstoneCore/ArchiveSceneModel.swift", "court": "ios/Godstone/Tests/GodstoneCoreTests/ReadinessT50Tests.swift", "swift_filter": "ReadinessT50Tests", "find": "        await model.load(.search(trimmed))\n        if epoch != mine { return }                       // stale after the road", "replace": "        await model.load(.search(trimmed))\n        if epoch != mine { }                       // (mutant) the gate sleepeth", "why": "the token is not revalidated after the road: an obsolete failure, wakened late, publisheth its woe over the younger tale. The after-gate witness condemneth -- the determinism law of T49, unenforced", "witness": "testAnObsoleteFailureCannotOverwriteNewerResults"},
    ...
    {"id": "T50-SM8-blank-became-petition", "platform": "swift", "file": "ios/Godstone/Sources/GodstoneCore/ArchiveSceneModel.swift", "court": "ios/Godstone/Tests/GodstoneCoreTests/ReadinessT50Tests.swift", "swift_filter": "ReadinessT50Tests", "find": "        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)\n        if trimmed.isEmpty {\n            backToDocuments()\n            return\n        }", "replace": "        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)\n        if false, trimmed.isEmpty {\n            backToDocuments()\n            return\n        }", "why": "the blank petition ceaseth to be a return: whitespace searcheth as though it were a word, and the journey loseth its root. The blank-and-bounded witness condemneth", "witness": "testTheBlankPetitionIsAReturnAndTheFieldIsBounded"},

...
SEMANTIC = [
    ...
    {"id": "T53-SM15-malformed-door-wide", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t53.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t53.py", "find": "    except (OSError, ValueError) as exc:\n", "replace": "    except OSError as exc:\n", "why": "malformed JSON is refused at the door with a cried message and exit 1; narrowed to the OSError arm, a rotting document stealeth through as an uncaught traceback: the exit code still falleth nought but the message is never spoke, and the door witness condemneth", "witness": "testMalformedJSONIsRefusedAtTheDoor"},

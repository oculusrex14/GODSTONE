...
SEMANTIC = [
    ...
    {"id": "T49-SM10-restore-amnesial-mode", "platform": "jvm", "module": "app", "test_task": "testLightDebugUnitTest", "file": "android/app/src/main/java/io/godstone/app/ui/browse/BrowseViewModel.kt", "court": "android/app/src/test/java/io/godstone/app/readiness/ReadinessT49Test.kt", "gradle_filter": "*ReadinessT49Test*", "find": "handle[\"mode\"] = s.mode.name", "replace": "handle[\"mode\"] = BrowseMode.DOCUMENTS.name", "why": "the recreation witness forgetteth the scene: a journey interrupted in the document standeth reposed at the root. The recreation witness condemneth", "witness": "testProcessRecreationRestorethTheSameJourney"},
    {"id": "T49-SM11-restore-amnesial-identity", "platform": "jvm", "module": "app", "test_task": "testLightDebugUnitTest", "file": "android/app/src/main/java/io/godstone/app/ui/browse/BrowseViewModel.kt", "court": "android/app/src/test/java/io/godstone/app/readiness/ReadinessT49Test.kt", "gradle_filter": "*ReadinessT49Test*", "find": "handle[\"openedDocumentId\"] = s.openedDocumentId", "replace": "handle[\"openedDocumentId\"] = null", "why": "the opened document identity is struck from the recreation handle: the journey returneth stripped of whom it was reading. The recreation witness condemneth -- restored query AND document identity are the card's words", "witness": "testProcessRecreationRestorethTheSameJourney"},
    ...
    {"id": "T52-SM22-operator-refusal-muzzled", "platform": "python", "file": "scripts/prepare_release_assets.py", "court": "tools/readiness/tests/test_t52.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t52.py", "find": "    if \"archive_trust_store\" in data:\n", "replace": "    if False:\n", "why": "the T51 law in one line: the bundle may not choose its own trust root; muzzled upon the operator face, self-nomination passeth the gate", "witness": "testTheOperatorFaceStillRefusethSelfNomination"},
    {"id": "T52-SM23-dispatch-all-operator", "platform": "python", "file": "scripts/prepare_release_assets.py", "court": "tools/readiness/tests/test_t52.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t52.py", "find": "    if trust_store_path is not None:\n", "replace": "    if True:\n", "why": "the dispatch is the two faces of one gate; forced to the operator arm, the deputy's named guard and its whole vintage are never read", "witness": "testTheDeputyFaceReadethTheDocumentPointersLawfully"},
    {"id": "T52-SM24-deputy-required-guard-struck", "platform": "python", "file": "scripts/prepare_release_assets.py", "court": "tools/readiness/tests/test_t52.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t52.py", "find": "        if not manifest_ref or not trust_ref:\n", "replace": "        if False:\n", "why": "the deputy bindeth the same verifier and requireth both pointers; struck, a document without signed manifest or trust store is let pass to the verifier's confusion", "witness": "testTheDeputyFaceReadethTheDocumentPointersLawfully"},
    {"id": "T52-SM25-gates-alarm-sleepeth", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t52.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t52.py", "find": "    mutate(\"duplicate gate\", lambda value: value[\"gates\"].append(value[\"gates\"][0]))\n", "replace": "    mutate(\"duplicate gate\", lambda value: None)\n", "why": "the roll of twelve is the checker's own contract; strike one control from the roll and the counter, trusting his roll, returneth the false friend's nought while the duplicate cry is never heard -- the subprocess witness heareth the rc and dieth alone", "witness": "testTheGatesCheckerRefusethAllTwelveByName"},
    {"id": "T52-SM26-evidence-shape-widened", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t52.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t52.py", "find": "            if not isinstance(ev, str) or not re.fullmatch(r\"[0-9a-f]{40}\", ev) or ev == \"0\" * 40:\n", "replace": "            if not isinstance(ev, str) or not re.fullmatch(r\".{40}\", ev) or ev == \"0\" * 40:\n", "why": "a CLOSED gate's evidence is a full lowercase hexadecimal commit; widened to any forty bytes, the forged non-hex and upper-case shapes are no more refused -- the twelve originals still refuse by their own constructions, so the in-process oath upon the crafted shapes is the true witness and dieth alone", "witness": "testTheEvidenceShapeIsSwornFullLowercaseHex"},
    ...
    {"id": "T53-SM05-required-census-asleep", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t53.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t53.py", "find": "    for field in profile[\"required\"]:\n", "replace": "    for field in ():  # (mutant) the census of required fields sleepeth\n", "why": "the profiled closure must bring its whole evidence; with the census of required fields asleep a LIGHT closure may ride without its APK and AAB oaths and the corpus without its digest -- the control that loseth a checksum is double-armed (run id) and keepeth its refusal, and the profile-partition witness, who counteth the 'evidence wanteth' cries, condemns", "witness": "testTheProfilesKeepLightAndCorpusApart"},
    {"id": "T53-SM06-checksum-shape-blind", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t53.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t53.py", "find": "            if not isinstance(value, str) or not SHA256.fullmatch(value):\n", "replace": "            if False:  # (mutant) any digest is a digest to the blind eye\n", "why": "the digests must be full lower-case SHA-256; blinded, forty uppercase characters and a truncated oath pass for hex; the control of the truncated checksum is double-armed (executor shape) and keepeth its refusal, and the field-format witness condemns (his upper/truncated arms)", "witness": "testTheEvidenceFieldFormatsAreStrict"},
    ...
    {"id": "T53-SM11-skipped-job-countenanced", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t53.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t53.py", "find": "        elif re.search(r\"^\\s*if:\\s*false\\s*$\", block, re.M):\n", "replace": "        elif False:  # (mutant) a skipped job is counted as good friend\n", "why": "a job skipped by condition is UNAVAILABLE and may never be mapped to PASS; the selftest's skipped control is double-armed (its block loseth the inspection step too, so the must-hold arm refuseth still) and keepeth ringing, and the skipped-or-missing witness (his skipped arm) condemns", "witness": "testSkippedOrMissingJobMayNotClose"},
    ...
    {"id": "T53-SM25-legacy-veil-lifted", "platform": "python", "file": "ci/check_release_gates_status.py", "court": "tools/readiness/tests/test_t53.py", "py_dir": "tools/readiness/tests", "py_pattern": "test_t53.py", "find": "    if version == 1:\n        return errors\n", "replace": "    if version == 0:\n        return errors\n", "why": "the legacy (v1) style is kept byte-identical for the sealed courts' fixtures; with the early veil lifted, an unversioned fixture is judged by the v2 law also: the rogue gate is smitten that the legacy path should have passed in peace, and the byte-identical witness condemns (the twelve sealed controls carry all eight gates and are insensible to this stroke)", "witness": "testTheLegacyPathIsByteIdentical"},
    # ------------------------------------------------------- T61 (provenance)
    #

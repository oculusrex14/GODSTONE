    ...
    "GS-INTEGRATION-001": [
        ...
        {"id": "gs-integration-001.scenarios",
         ...
         "status": "PARTIAL", "evidence": [
             "*** (A) WRONG PEER/KEY AT THE REAL HANDSHAKE: ABSENT. *** *The court carrieth ZERO `pairUp`/HS1-3/keyConfirmation sites -- it useth the `barePair` registry (`path:ios/Godstone/Tests/GodstoneMeshTests/ReadinessTrustedPairing.swift:81`, 'no pairing run yet') and only EXISTING identities.* The obligation nameth this as the difference between proving handshake identity/key rejection and `FrameV2.decode`'s structural wire validation, **and the court's own record marketh it OWED.**",
             "*** (B) AND (C) DURABLE-RESTART: PARTIAL, AND THE SUBSTITUTION IS THE POINT. *** *MEASURED: the court's durable estate is IN-MEMORY -- `InMemoryMessageStore` (`path:ios/Godstone/Sources/GodstoneMesh/MessageStore.swift:3369`) at :789-790 and `InMemoryAckStore` (`path:ios/Godstone/Sources/GodstoneMesh/AckObligationStore.swift:357`) at :819-820 -- **and an in-memory object surviving a loop is not process/restart durability, which the obligation forbiddeth explicitly.*** **THE REAL DURABILITY WITNESSES EXIST ELSEWHERE AND OVER THE RIGHT AUTHORITIES:** `path:ios/Godstone/Tests/GodstoneMeshTests/SqliteMessageStoreTests.swift:932` (outbound enqueue + delivery row across a REAL file reopen) and `path:ios/Godstone/Tests/GodstoneMeshTests/CrashStartupResumeTests.swift:343` (the real ACK estate rebuilt after a real close/reopen over the same Sqlite file). ***WHAT IS NOT DONE IS DRIVING THOSE THROUGH THE TRANSPORT RIG -- so the scenarios are witnessed on the STORE authority and not yet on the INTEGRATION road.***",

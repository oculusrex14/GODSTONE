...
PARTIAL_OBLIGATIONS: dict[str, list[dict]] = {
    ...
    "GS-UX-001": [
        ...
        {"id": "gs-ux-001.ui-test-target",
         ...
         "status": "PARTIAL", "evidence": ["`path:ios/Godstone/Tests/LabMeshUITests/LabMeshUITests.swift`, `path:ios/Godstone/Tests/GodstoneArchiveUITests/GodstoneArchiveUITests.swift`, `path:ios/project.yml`, `path:.github/workflows/repository-verification.yml`. **TWO REAL `bundle.ui-testing` TARGETS, BOTH CI-GATED**: `LabMeshUI` (6 arms now) and the new `GodstoneArchiveUI` (6 arms). *`LabMeshUI` was invoked by NO GATE anywhere until an external review found it.* *** THE FULL JOURNEY LIST IS NOT YET COVERED, WHICH IS WHY THIS IS PARTIAL RATHER THAN DISCHARGED: *** *the SOS hold AND accessible alternative are covered (`testGSINT001TheSosJourneyRendersAControlRatherThanALabel`), and **CANCEL IS NOW COVERED TOO** (`testGSINT001ACancelledSosHoldDoesNotReachTheRuntime`, added this round and MUTATION-PROVEN: removing the confirmation threshold REDDENS it on exactly its own assertion, `*** A LIFTED HOLD MUST BE REPORTED AS CANCELLED ***`, with `Executed 6 tests, with 0 failures` restored). **STILL OWED: the recipient/trust/wipe controls are individually covered but several of the card's named journeys are not driven end to end in ONE arm, and the archive target's recreation arm is deterministically RED** -- see `gs-archive-005.app-witness`.*"]},

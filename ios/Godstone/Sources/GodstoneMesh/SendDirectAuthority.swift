...
public final class SendDirectAuthority: @unchecked Sendable {
    ...
    public func sendDirect(_ command: SendDirectCommand, priority: Priority) async -> SendDirectResult {
        ...
        // *** CRYPTO-005 (round 470): **FAIL CLOSED.** The loaded outcome is TYPED, and a FAULT IS NOT ABSENCE: folding
        // `.storageFailure` or `.corrupt` into "not found" would let a RETRY BECOME A NEW SEND -- the very defect this finding is
        // about. The rejection case used is the canonic-mismatch one, WHICH IS THE NEAREST EXISTING REFUSAL rather than a new case
        // invented here; a dedicated case is owed and NAMED in this finding's pending proof.
